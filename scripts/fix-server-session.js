...
/**
 * This script fixes the getServerSession method calls in API routes
 */
...
apiFiles.forEach(filePath => {
  try {
    if (fs.existsSync(filePath)) {
      ...
      if (content.includes('auth.getServerSession')) {
        console.log(`Processing ${filePath}...`);
        
        // Update auth.getServerSession to use the correct method in Better Auth
        const updatedContent = content
          .replace(/auth\.getServerSession\(\)/g, 'auth.getSession()');
